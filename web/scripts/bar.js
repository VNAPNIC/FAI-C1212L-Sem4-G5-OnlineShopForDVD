	...
	$.extend({
		progressBar: new function() {
			...
			this.defaults = {
				...
				width			: 514,											// Width of the progressbar - don't forget to adjust your image too!!!												// Image to use in the progressbar. Can be a single image too: 'images/progressbg_green.gif'
				height			: 24,											// Height of the progressbar - don't forget to adjust your image too!!!
				callback		: null,											// Calls back with the config object that has the current percentage, target percentage, current image, etc
				boxImage		: 'images/progress_over.png',						// boxImage : image around the progress bar
				barImage		: {
			...
			this.construct = function(arg1, arg2) {
				...
				return this.each(function(child) {
					...
					if (argvalue != null && this.bar != null && this.config != null) {
						...
						$bar.css("background-position", ((config.width * -1)) + 'px 100%');
						$bar.css("padding", "0");
						$bar.css("margin", "0");
						$this.append($text);
						$this.append($bar);
					...
					function getText(config) {
						if (config.showText) {
							if (config.textFormat == 'percentage') {
								return " " + Math.round(config.running_value) + "%" + " complete!";
					...
					var t = setInterval(function() {
						...
						$bar.css("background-position", (((config.width * -1)) + (getPercentage(config) * pixels)) + 'px 100%');
						$bar.attr('title', getText(config));
						$text.html(getText(config));
